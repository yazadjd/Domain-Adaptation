# -*- coding: utf-8 -*-
"""OneHotEncoding of SML_Project2_Female.ipynb

Automatically generated by Colaboratory.

This notebook contains code relevant to running all baselines using one hot encoded features for target domain = FEMALE for Task 1.1 of SML Project 2 - Sem 1, 2020.
"""
...
x_train = pd.concat([male_X_train, mixed_X_train])
y_train = pd.concat([male_Y_train, mixed_Y_train])

lin_reg(x_train, y_train, female_X_val, female_Y_val)
lin_reg(x_train, y_train, female_X_test, female_Y_test)

# MLPRegressor(random_state = 42, max_iter = 1000, activation = 'relu', solver = 'adam', hidden_layer_sizes=h, learning_rate='constant', learning_rate_init= 0.015)
MLP_Reg(x_train, y_train, female_X_val, female_Y_val, 50)
MLP_Reg(x_train, y_train, female_X_test, female_Y_test, 50)

"""TGTONLY"""

x_train = sample(female_X_train)
y_train = sample(female_Y_train)
...
MLP_Reg(x_train, y_train, female_X_val, female_Y_val, 42)
MLP_Reg(x_train, y_train, female_X_test, female_Y_test, 42)
...
x_train = pd.concat([sample(female_X_train)]*85, ignore_index=True)
x_train = pd.concat([x_train, x_train.head(54), male_X_train, mixed_X_train], ignore_index = True)
y_train = pd.concat([sample(female_Y_train)]*85, ignore_index=True)
y_train = pd.concat([y_train, y_train.head(54), male_Y_train, mixed_Y_train], ignore_index = True)

lin_reg(x_train, y_train, female_X_val, female_Y_val)
lin_reg(x_train, y_train, female_X_test, female_Y_test)

# MLPRegressor(random_state = 42, max_iter = 1000, activation = 'relu', solver = 'adam', hidden_layer_sizes=h, learning_rate='constant', learning_rate_init= 0.001)
MLP_Reg(x_train, y_train, female_X_val, female_Y_val, 12)
MLP_Reg(x_train, y_train, female_X_test, female_Y_test, 12)

all = []
for n in range(1,200):
  all.append((MLP_Reg(x_train, y_train, female_X_val, female_Y_val, n), n))
...
def DT(x_train, y_train, x_val, y_val, n):
  tree_model = DecisionTreeRegressor(random_state=42, splitter = 'best', max_depth = n, max_features='auto')
  tree_model.fit(x_train, y_train)
  DT_pred = tree_model.predict(x_val)
  tree_test_mse = mean_squared_error(y_val, DT_pred)
  tree_test_mae = mean_absolute_error(y_val, DT_pred)
  print("Decision Tree test mse = ",tree_test_mse," & mae = ",tree_test_mae," & rmse = ", sqrt(tree_test_mse), 'n = ', n)
  return tree_test_mse
  #return DT_pred
...
predictions = lin_reg(x_train, y_train, female_features, female_scores)
augmented_female_features = female_features.assign(src_preds = predictions)
X_train, female_X_test, y_train, female_Y_test = train_test_split(augmented_female_features, female_scores, test_size = 0.2, random_state = 42)
female_X_train, female_X_val, female_Y_train, female_Y_val = train_test_split(X_train, y_train, test_size = 100, random_state = 42)
x_train = sample(female_X_train)
y_train = sample(female_Y_train)
...
#MLPRegressor(random_state = 42, max_iter = 1000, activation = 'relu', solver = 'adam', hidden_layer_sizes=h, learning_rate='constant', learning_rate_init= 0.001)
predictions = MLP_Reg(x_train, y_train, female_features, female_scores, 50)
augmented_female_features = female_features.assign(src_preds = predictions)
X_train, female_X_test, y_train, female_Y_test = train_test_split(augmented_female_features, female_scores, test_size = 0.2, random_state = 42)
female_X_train, female_X_val, female_Y_train, female_Y_val = train_test_split(X_train, y_train, test_size = 100, random_state = 42)
x_train = sample(female_X_train)
...
# MLPRegressor(random_state = 42, max_iter = 1000, activation = 'relu', solver = 'adam', hidden_layer_sizes=h, learning_rate='constant', learning_rate_init= 0.001)
MLP_Reg(x_train, y_train, female_X_val, female_Y_val, 8)
MLP_Reg(x_train, y_train, female_X_test, female_Y_test, 8)
...
y_train = pd.concat([male_Y_train, mixed_Y_train])
# MLPRegressor(random_state = 42, max_iter = 1000, activation = 'relu', solver = 'adam', hidden_layer_sizes=h, learning_rate='constant', learning_rate_init= 0.001)
srconly_preds = MLP_Reg(x_train, y_train, female_X_val, female_Y_val, 50)
srconly_preds_test = MLP_Reg(x_train, y_train, female_X_test, female_Y_test, 50)

x_train = sample(female_X_train)
y_train = sample(female_Y_train)
#MLPRegressor(random_state = 42, max_iter = 1000, activation = 'relu', solver = 'adam', hidden_layer_sizes=h, learning_rate='constant', learning_rate_init= 0.001)
tgtonly_preds = MLP_Reg(x_train, y_train, female_X_val, female_Y_val, 42)
tgtonly_preds_test = MLP_Reg(x_train, y_train, female_X_test, female_Y_test, 42)
data = {'y1' : srconly_preds, 'y2 - y1' : np.absolute(tgtonly_preds - srconly_preds)}
x_train = pd.DataFrame(data)
y_train = female_Y_val
...
print(metrics.mean_squared_error(female_Y_test, lin_pred))

x_train = pd.concat([male_X_train, mixed_X_train])
y_train = pd.concat([male_Y_train, mixed_Y_train])
srconly_preds = lin_reg(x_train, y_train, female_X_val, female_Y_val)
srconly_preds_test = lin_reg(x_train, y_train, female_X_test, female_Y_test)

x_train = sample(female_X_train)
y_train = sample(female_Y_train)

tgtonly_preds = lin_reg(x_train, y_train, female_X_val, female_Y_val)
tgtonly_preds_test = lin_reg(x_train, y_train, female_X_test, female_Y_test)
#data = {'y1' : srconly_preds, 'y2 - y1' : np.absolute(tgtonly_preds - srconly_preds)}
